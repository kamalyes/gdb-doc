...
func getDbInfo(db *sql.DB) model.DbInfo {
	...
	// 表数量
	var sqlQuery = "select table_schema, count(*) as 'count' from information_schema.tables WHERE table_schema = "
	var sqlGroup = "\tgroup by table_schema;"
	var db_name = fmt.Sprintf("'%s'", dbConfig.Database)
	sql := sqlQuery + db_name + sqlGroup
	...
	info.TableNumber = value

	return info
}

// getPGDbInfo 获取pg数据库的基本信息
func getPGDbInfo(db *sql.DB) model.DbInfo {
	...
	// 表数量
	rows, err = db.Query("select count(tablename) from pg_tables; ")
	if err != nil {
		fmt.Println(err)
	}
	defer rows.Close()
