...
func createOfflineDoc(docPath string, dbInfo model.DbInfo, tables []model.Table) {
	...
	docMdArr = append(docMdArr, "| 数据库名称 | 版本 | 字符集 | 排序规则 | Table数量 |")
	docMdArr = append(docMdArr, "| ---- | ---- | ---- | ---- | ---- |")
	docMdArr = append(docMdArr, fmt.Sprintf("| %s | %s | %s | %s | %s | %s |", dbInfo.DbName, dbInfo.Version, dbInfo.Charset, dbInfo.Collation, dbInfo.TableNumber))
	// 生成目录
